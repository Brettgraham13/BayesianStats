...
final <- "model{
  #Data
  for(i in 1:length(H_pts)) {
    H_pts[i] ~ dpois(off*H_off[i] + def*A_def[i] + h0[home[i]])
    A_pts[i] ~ dpois(off*A_off[i] + def*H_def[i])
  
    log(H_off[i]) <- b0_o + b1_o[w[i]]*(h_O[i]) + b2_o[w[i]]*(hprior_O[i]) 
    log(A_off[i]) <- b0_o + b1_o[w[i]]*(a_O[i]) + b2_o[w[i]]*(aprior_O[i]) 

    log(H_def[i]) <- b0_d + b1_d[w[i]]*(h_D[i]) + b2_d[w[i]]*(hprior_D[i]) 
    log(A_def[i]) <- b0_d + b1_d[w[i]]*(a_D[i]) + b2_d[w[i]]*(aprior_D[i]) 
  }
  ...
  for(j in 1:32){
    h0[j] ~ dnorm(hmu,1/htau)
    # a0[j] ~ dnorm(amu,1/atau)
    # team[j] ~ dnorm(tmu,1/ttau)
  }
...
single_prediction <- function(modelDFRow, predictionsDF){
  home_pred = rpois(10000, predictionsDF$b00 +  predictionsDF[,modelDFRow$Home_id + 9] + exp(predictionsDF$b0 + predictionsDF[,modelDFRow$binnedWeek + 2]*(modelDFRow$`Home_O_Points` - modelDFRow$`Away_D_Points`) + predictionsDF[,modelDFRow$binnedWeek + 5]*(modelDFRow$`Home_O_Points_Last_Year` - modelDFRow$`Away_D_Points_Last_Year`)))
...
```{r}
set.seed(13)
predict2017 = all_predictions(modelDF2017, final_sample)
...
buffalo = teamPredictions(predict2017, "Buffalo")
...
ggplot(buffalo, aes(x= Week,y = `Point Differential`,color = Result)) +
    geom_line() + geom_point()+ ggtitle("Buffalo")
